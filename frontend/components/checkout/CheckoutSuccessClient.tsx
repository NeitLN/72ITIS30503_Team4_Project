...
import { tPaymentMethod, tStatus } from '../../lib/i18n';
  ...
  return (
    <Container className="py-12 sm:py-20 max-w-3xl">
      <div className="border border-neutral-900 bg-white p-6 sm:p-10 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
        ...
        <div className="mt-7 border border-neutral-200 bg-neutral-50 p-4 text-sm"><strong>Thanh toán: {tPaymentMethod(order.payment_method)}</strong><p className="mt-1 text-neutral-600">{order.payment_method === 'cod' ? 'Bạn sẽ thanh toán khi nhận hàng.' : 'Đơn đã được tạo; hãy làm theo hướng dẫn chuyển khoản của StyleHub. Trạng thái thanh toán chưa được xác nhận tự động.'}</p></div>
